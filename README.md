...
- `SIM6`-`SIM10` — **fabricated data**, generated by
  `scripts/generate_synthetic_sessions.py` to bulk out the group-average
  comparison charts (Control/ADHD/Autistic) beyond n=1-3 per group. Raw
  signals (rPPG BPM/RMSSD, gaze, conflict-task trials) are simulated from
  group-conditioned parameters and run through this repo's own
  `csi_cvi_pipeline.py` math, not hand-picked summary numbers — but they are
  still synthetic, not participant data. Re-run the script (it overwrites
  `SIM*.csv`) if you want a different pool size or group split.
