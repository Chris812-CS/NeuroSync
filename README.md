...
`testing data/` holds two kinds of session CSVs, both in the same real
export format:

- `T1`-`T5` — real pilot recordings.
- `T6`-`T15` — **fabricated data**, generated by
  `scripts/generate_synthetic_sessions.py` to balance the group-average
  comparison charts to 5 Control / 5 ADHD / 5 Autistic (the real pool alone
  is Control 3, Autistic 2, ADHD 0). Raw signals (rPPG BPM/RMSSD, gaze,
  conflict-task trials) are simulated from group-conditioned parameters and
  run through this repo's own `csi_cvi_pipeline.py` math, not hand-picked
  summary numbers — but they are still synthetic, not participant data.
  They deliberately use the same `T#` filename pattern as the real
  recordings (no `SIM`-style marker) so they render identically in the
  dashboard; this file and the generator script are the only record of
  which sessions are fabricated. Re-run the script (it overwrites
  `T6.csv`-`T15.csv`) if you want a different pool size or group split.
